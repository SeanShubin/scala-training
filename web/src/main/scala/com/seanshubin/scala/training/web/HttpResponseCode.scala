...
/*
Scala does not have enums built into the language.

As an alternative, you can try
(1) case classes that extend a sealed trait in the same file
(2) the scala.Enumeration class
(3) The pattern shown here

Here is a pattern I like to use that meets the following criteria
(1) statically typed
(2) can iterate through all values
(3) new values need only be added in one place
(4) can use polymorphic methods (not shown here)

This pattern is probably overkill for http response codes,
but I wanted to show a possible implementation of enums in scala
*/
